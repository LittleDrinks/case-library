...
test_skill_platform.py 的并发上传用例已迁移至 test_skill_platform_e2e.py（真实 MongoDB replica set，经既有 E2E 应用 HTTP 公共接口执行），断言原样保留：并发上传各得唯一 v1..vN，latest 不回退。RAR用例统一使用 backend/tests/fixtures/materials.rar 专用两条目样本，宿主与容器均由测试路径相对读取；既有真实 RAR 解析与对象存储验收保留。
执行状态：9a0124d 记录的轻量层 60 个中文场景已实际执行并全部通过；本轮 reviewer 修复后未重跑 60，不将旧结果视为本轮验收。backend-e2e 的中文 search_sync 场景扩展为公开发布后检索命中、下线后轮询消失，代码已接入，真实容器尚未执行；中文 JUnit/HTML 报告出口已由独立集成提交接入，实际报告待隔离执行。浏览器切片 4 片（登录会话、编辑自动保存、恢复取消对话框、DOCX下载链）待做：固定为真实 .feature + 既有 Playwright/JS Gherkin 绑定另做，未完成的 Python 浏览器文件不进本切片。undo_sync 场景写入前提为真空正文，断言撤销恢复正文完整结构相等、重复撤销严格 200 且修订号不变、正文变化后首次撤销严格 409 且写入保持 written。search_sync 场景经 e2e-app 真实 HTTP 与常驻 search-worker 消费 outbox 后的公开 /api/search 观察，先证明发布命中，再证明下线消失；真实权限检索使用既有 test_search_case_content_e2e 的公开/校内/私密内容合同，不以轻量恒空替身冒充真实权限验收。backend/tests/fixtures/materials.rar 为专用两条目样本，已由质量槽验证；旧 assets/学习资料md.rar 与 /app/fixtures fallback 已移除。
