...
| 子系统（PRD v2） | 复用目标 | 决策 |
| --- | --- | --- |
| Agent 工具环 | `pydantic-ai` | typed 工具注册、结构化输出（写作候选/候选批注）、OpenAI 兼容 base_url、流式事件；pydantic 2 与现栈同构。工具即现有服务函数薄封装：`search_corpus`=search.service，`fetch_url`=trafilatura 抽取后走 attachments 快照存储，写工具沿用 ADR 0016 语义：默认候选确认，#214 起教师消息明确直接写入且服务端授权校验通过时可 `write_document` 直接执行并保留撤销；修订阻塞、批前快照是应用层状态机，不进框架 |
...
6. pydantic-ai agent 环：三个读工具 + 写工具（默认确认门；#214 起支持服务端授权的直接写入与撤销，ADR 0016 语义，workbench e2e）
