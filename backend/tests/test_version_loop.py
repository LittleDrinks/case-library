...
def _save_content(client, auth: dict, case: dict, text: str) -> dict:
    response = client.patch(
        ...
        json={**document_update(case, paragraph_document(text))},
    )
...
def _frozen_version(client, auth: dict, marker: str):
    ...
    created = client.post(
        "/api/cases",
        headers={"X-CSRF-Token": auth["csrfToken"]},
        json={"title": marker, "document": paragraph_document(f"{marker} 冻结正文")},
    ).json()
...
def _draft_annotation(client, auth: dict, case: dict, quote: str, note: str) -> dict:
    """quote 需为正文 utf16 切片；剩余 409 为纯段落文档 section 计算差异（WIP）。"""
    response = client.post(
        f"/api/cases/{case['id']}/annotations",
        headers={"X-CSRF-Token": auth["csrfToken"]},
        json={
            "quote": quote, "section": "正文", "content": note, "source": "manual",
            "revision": case["revision"],
            "from": 2, "to": 2 + len(quote.encode("utf-16-le")) // 2,
...
def _version_annotation(client, auth: dict, case: dict, version_id: str) -> dict:
    """投稿→管理员开审挂版本批注→作者撤回；返回撤回后的工作稿。"""
...
def test_restore_keeps_current_work_and_appends_restore_record(client: TestClient) -> None:
    ...
    version_note = _version_annotation(client, auth, changed, version["id"])
    ...
    assert kinds == ["submission", "manual", "restore"]
    baseline = rows[1]
    assert baseline["title"] == "恢复前的当前稿"
    assert baseline["document"] == changed["document"]

    restore_record = rows[2]
    assert restore_record["title"].startswith("恢复：")
    assert restore_record["document"] == version["document"]
    assert restore_record["restoredFromId"] == version["id"]
...
def test_restored_annotations_keep_discussion_state(client: TestClient) -> None:
    ...
    note = _draft_annotation(client, auth, case, "状态恢复 冻结正文", "待回复批注")
    client.post(
