...
def _seed_case_source(database, case_id: str) -> None:
    ...
    database.case_sources.insert_one({
        "id": "src-a", "sourceType": "case", "caseId": case_id,
...
EVIDENCE = SourceRef(
    kind="case", id="c-src", title="来源案例 v1", version="v1",
    version_id="cv-1", locator="case:c-src@cv-1",
)


def _seed_evidence_case(client: TestClient, auth: dict):
    """构造完成态 Run + 引用冻结来源的 pending Artifact，供接受复验。"""
    case = _create_case(client, auth, *PARAGRAPHS)
    database, repository, thread, run = _locked_run(client, auth, case)
    _seed_case_source(database, case["id"])
    artifact = _propose(database, case, run, sources=[EVIDENCE])
    assert repository.complete_run(run.id, _assistant_message(run), resources=[]) is True
    return database, case, thread, artifact


def _refused_accept(client, database, case, thread, artifact, auth):
    with pytest.raises(CaseError) as excinfo:
        artifacts.decide_artifact(
            database, case["id"], thread.id, artifact.id, auth["user"], "accepted",
...
def _set_source(database, **fields) -> None:
    database.cases.update_one({"id": "c-src"}, {"$set": fields})


def test_accept_rechecks_evidence_permission_and_frozen_version(client: TestClient) -> None:
    auth = _login(client)
    database, case, thread, artifact = _seed_evidence_case(client, auth)
    _set_source(database, publicationStatus="private")
    denied = _refused_accept(client, database, case, thread, artifact, auth)
    assert denied.value.status_code == 409
    assert database.agent_artifacts.find_one({"id": artifact.id})["status"] == "pending"
    assert database.cases.find_one({"id": case["id"]})["revision"] == 1
    _set_source(database, publicationStatus="public", publishedVersionId="cv-2")
    drifted = _refused_accept(client, database, case, thread, artifact, auth)
    assert drifted.value.status_code == 409
    _set_source(database, publishedVersionId="cv-1")
    result = artifacts.decide_artifact(
        database, case["id"], thread.id, artifact.id, auth["user"], "accepted",
        store=client.app.state.blob_store,
    )
    ...
    assert current["revision"] == 2 and REPLACEMENT in str(current["document"])
