...
def test_expired_owner_cannot_publish_after_a_new_owner_takes_over() -> None:
    ...
    assert queue.publish("new-rebuild", _marker("new", "catalog-new", "epoch-new", ["catalog-old"])) is True
    assert queue.publish("old-rebuild", _marker("old", "catalog-old", "epoch-old", [])) is False
    assert queue.target().generation == "new"


def _marker(generation, index_uid, epoch, retired) -> dict:
    return {"_id": "catalog", "generation": generation, "indexUid": index_uid, "indexEpoch": epoch, "retiredIndexUids": retired}
