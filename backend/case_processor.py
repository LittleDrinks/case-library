...
# 导入数据库模块
from database import create_case, update_case, get_case
...
def classify_case(content: str) -> Dict:
    """调用分类器判断案例类型"""
    try:
        classifier_path = os.path.join(SKILL_DIR, 'classifier.md')
        ...
        if os.path.exists(classifier_path):
            ...
            result = _simple_classify(content)
            return result
        else:
            return {
                'primary_type': 'TYPE_A',
...
def _simple_classify(content: str) -> Dict:
    ...
    if any(keyword in content_lower for keyword in ['习近平', '新时代', '社会主义', '价值观', '典型人物', '先进事迹', '英雄', '榜样']):
        types.append('TYPE_A')
    
    # TYPE_B判断：课程、教学、专业
    if any(keyword in content_lower for keyword in ['课程', '教学', '专业', '课堂', '实验室', '实习', '毕业设计']):
        types.append('TYPE_B')
    
    # TYPE_C判断：实践、志愿、服务
    if any(keyword in content_lower for keyword in ['实践', '志愿', '服务', '社区', '农村', '援疆', '帮扶', '劳动']):
        types.append('TYPE_C')
    ...
    # 主题分类
    if any(keyword in content_lower for keyword in ['国家战略', '科技', '芯片', '航天', '新能源', '人工智能', '援疆', '乡村振兴']):
        themes.append('强国建设')
    
    if any(keyword in content_lower for keyword in ['上海', '五个中心', '长三角', '城市治理', '文化']):
        themes.append('上海实践')
    
    if any(keyword in content_lower for keyword in ['创新', '科技攻关', '专利', '成果转化', '3D打印', 'AI']):
        themes.append('创新发展')
...
def get_template(type_code: str) -> Optional[str]:
    """获取模板"""
    template_map = {
    ...
    if template_name:
        template_path = os.path.join(SKILL_DIR, template_name)
