...
function scheduleWords(view, offset) {
...
function compressionRound(state, words) {
...
function fallbackHash(value) {
  ...
  let state = Uint32Array.from(SHA256_INITIAL);
  const view = new DataView(padded.buffer);
  for (let offset = 0; offset < padded.length; offset += 64) {
    const round = compressionRound(state, scheduleWords(view, offset));
    state = state.map((value, index) => (value + round[index]) >>> 0);
  }
  return Array.from(state, (word) => word.toString(16).padStart(8, "0")).join("");
}

export async function hashQuote(value) {
  const bytes = new TextEncoder().encode(value);
