...
<template>
  <section class="search-graph" role="region" aria-label="当前检索结果图谱">
    ...
    <aside v-if="selected?.item" class="graph-selection" aria-live="polite">
      ...
      <RouterLink v-else-if="selected.type === 'material' && selected.item.contentAvailable" :to="{ name: 'material-detail', params: { id: selected.item.id } }">查看素材<ExternalLink :size="14" /></RouterLink>
    </aside>
