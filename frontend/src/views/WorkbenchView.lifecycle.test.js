...
test("草稿作者的动作按钮由服务端 availableActions 驱动", async () => {
  ...
  expect(wrapper.find('button[aria-label="另开新稿"]').exists()).toBe(false);
});
...
test("已发布作者看到另开新稿与公开页入口，另开后回到可编辑草稿", async () => {
  const wrapper = await renderCase({
  ...
  api.lifecycleCase.mockResolvedValue({
    case: caseFixture({ revision: 4, publicationStatus: "public" }), version: {}, event: {},
  });
  await wrapper.get('button[aria-label="另开新稿"]').trigger("click");
  await flushPromises();
  expect(api.lifecycleCase).toHaveBeenCalledWith(
    "case-1", { command: "reopen", revision: 3 }, "csrf-token",
  );
  expect(wrapper.find('button[aria-label="提交审核"]').exists()).toBe(true);
  expect(wrapper.text()).toContain("草稿 · 旧版公开中");
});
...
test("提交校验失败展示服务端消息", async () => {
  ...
  api.lifecycleCase.mockRejectedValue(
    Object.assign(new Error("标题不能为空；必填标签组未选择标签：学科"), { status: 422 }),
  );
  await wrapper.get('button[aria-label="提交审核"]').trigger("click");
  await flushPromises();
  expect(wrapper.text()).toContain("标题不能为空；必填标签组未选择标签：学科");
});
