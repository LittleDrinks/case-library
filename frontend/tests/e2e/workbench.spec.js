...
test("附件请求在途锁定正文避免与自动保存竞争", async ({ page }) => {
  ...
  await page.locator(".canvas-editor p").filter({ hasText: marker }).click();
  await page.keyboard.press("End");
  await page.keyboard.type(" upload finished");
  await expect(page.locator(".save-state")).toHaveText("已保存");
  await page.reload();
  await expect(page.locator(".canvas-editor")).toContainText("upload finished");
});
